...
class InputExampleWiki80_ossl2_tail(object):
    """A single training/test example for 给定头实体和关系类别，预测尾实体。"""
    def __init__(self, guid, sentence, span1, ner1, relation, label):
        self.guid = guid
        self.sentence = sentence
        self.span1 = span1 ### 头实体的位置
        self.ner1 = ner1
        self.relation = relation ### 关系的文字 字符串
        self.label = label # （尾实体的文字）字符串
...
class wiki80Processor(DataProcessor):
    ...
    def __init__(self, data_path, use_prompt, ossl2_label_type):
        super().__init__()
        self.data_dir = data_path
        self.ossl2_label_type = ossl2_label_type
    ...
    def _create_examples(self, dataset, set_type):
        ...
        if self.ossl2_label_type == 'tail':

            with open('./dataset/ossl2/tail_clustering_dict.json', 'r', encoding='utf8')as fp:
                tail_clustering_dict = json.load(fp)

            examples = []
            for example in dataset:
                sentence = example['token']
                # （将 尾实体的字符串 进行 弱映射）
                tail = ' '.join(example['token'][example['t']['pos'][0]: example['t']['pos'][1]])
                tail = tail_clustering_dict[tail]
                # 组装
                examples.append(InputExampleWiki80_ossl2_tail(guid=None,
                                                   sentence=sentence,
...
def convert_examples_to_features(examples, max_seq_length, tokenizer, args, rel2id):
    ...
    save_file = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))+'/'+"dataset/cached_wiki80.pkl"
    mode = "text"
    ...
    use_bert = "BertTokenizer" in tokenizer.__class__.__name__ # no (but "robertaXXX")
    use_gpt = "GPT" in tokenizer.__class__.__name__ # no
    ...
    if False:
        ...
        print('Successfully converted instances:', len(instances))
...
def convert_examples_to_features_ossl2_tail(examples, max_seq_length, tokenizer, args, rel2id):
    """Loads a data file into a list of `InputBatch`s."""
    ...
    if False:
        ...
        for (ex_index, example) in enumerate(examples):
            ...
            """
                the SUBJECT + relation >> OBJECT is .
            ...
            if mode.startswith("text"):
                for i, token in enumerate(example.sentence):
                    ...
                    tokens.append(token)
                    # 当前的tokens：原句，以及对应实体位置加了限定的[start/end]
            ...
            prompt = f"[sub] {SUBJECT} [sub] {RELATION} [obj] {tokenizer.mask_token} [obj] ."


            if ex_index == 0:  # （在上面处理完毕后，输出第一个样例看看）
                input_text = " ".join(tokens)
                logger.info(f"input text : {input_text}")
                logger.info(f"prompt : {prompt}")
                logger.info(f"label : {example.label}")
            ...
            for i in range(len(inputs['input_ids'])):
                if sub_st == -1 and inputs['input_ids'][i:i + len(SUBJECT_ids)] == SUBJECT_ids:
                    sub_st = i
                    sub_ed = i + len(SUBJECT_ids)
            # （从生成的inputs['input_ids']里面，重新找到sub/obj的对应位置。）
            obj_st = 0
            obj_ed = len(inputs['input_ids'])-1
...
def get_dataset(mode, args, tokenizer, processor):
    ...
    if "wiki80" in args.task_name and "bart" not in args.model_name_or_path and "t5" not in args.model_name_or_path:
        # #############
        if args.ossl2_label_type == 'tail':
            dataset = convert_examples_to_features_ossl2_tail(
