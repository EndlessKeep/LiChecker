...
class DIALOGUE(BaseDataModule):
    ...
    def add_to_argparse(parser):
        ...
        roberta_path = 'D:\\PythonProject\\LiResolver_copy\\RE\\dataset\\ossl2\\roberta-base'
        parser.add_argument("--task_name", type=str, default="normal", help="[normal, reloss, ptune]")
        parser.add_argument("--model_name_or_path", type=str, default=roberta_path, help="Number of examples to operate on per forward step.")
...
class WIKI80(BaseDataModule):
    def __init__(self, args, model=None) -> None:
        super().__init__(args)
        self.dir = 'D:\\PythonProject\\LiResolver_copy\\RE\\dataset\\ossl2'
        self.args.max_seq_length = getattr(args, "max_seq_length", 256)
        self.processor = processors[self.args.task_name](self.args.data_dir, self.args.use_prompt, self.args.ossl2_label_type)
        self.tokenizer = AutoTokenizer.from_pretrained(self.args.model_name_or_path)
        self.data_train = self.processor.get_train_examples(self.dir)  # 加载训练数据
        self.data_val = self.processor.get_dev_examples(self.dir)  # 加载验证数据
        self.data_test = self.processor.get_test_examples(self.dir)
        self.collate_fn = partial(self._collate_fn_wiki80, tokenizer=self.tokenizer)
    ...
    @staticmethod
    def _collate_fn_wiki80(batch, tokenizer):
        input_ids = [item[0] for item in batch]
        attention_mask = [item[1] for item in batch]
        labels = [item[2] for item in batch]
        so = [item[3] for item in batch]

        # 确保每个元素的形状正确
        for i, ids in enumerate(input_ids):
            if not isinstance(ids, torch.Tensor):
                raise ValueError(f"input_ids[{i}] is not a tensor, got {type(ids)}")
            if len(ids.shape) != 1:
                raise ValueError(f"input_ids[{i}] should be 1D tensor, got shape {ids.shape}")
    ...
    def add_to_argparse(parser):
        ...
        parser.add_argument("--model_name_or_path", type=str, default = roberta_path, help="Number of examples to operate on per forward step.") # /home/xx/bert-base-uncased
        parser.add_argument("--max_seq_length", type=int, default=256, help="Number of examples to operate on per forward step.") # 512
        parser.add_argument("--ptune_k", type=int, default=7, help="number of unused tokens in prompt")
        return parser

    def train_dataloader(self):
        dataloader = DataLoader(self.data_train, shuffle=True, batch_size=self.batch_size, num_workers=0,
                                pin_memory=False,collate_fn=self.collate_fn)
        # collate_fn的作用是定义batch返回的数据格式
        return dataloader
