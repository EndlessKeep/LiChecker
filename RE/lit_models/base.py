...
class BaseLitModel(pl.LightningModule):
    ...
    def add_to_argparse(parser):
        ...
        parser.add_argument("--lr", type=float, default=3e-5) # LR
    ...
    def on_train_start(self):
        """在训练开始时动态替换调度器，此时 self.trainer 可用来计算 num_training_steps"""
        scheduler = get_linear_schedule_with_warmup(
            optimizer=self.optimizers(),
            num_warmup_steps=int(self.num_training_steps * 0.1),
    ...
    def num_training_steps(self) -> int:
        if not hasattr(self, "trainer") or self.trainer is None:
            # 默认值：若 trainer 尚未注入，返回一个合理的默认值（根据 max_epochs 推断）
            return self.args.get("max_epochs", 10) * 100
        ...
        print( print(f"DEBUG: num_training_steps = {max_estimated_steps} (Trainer exists: {hasattr(self, 'trainer')})"))
