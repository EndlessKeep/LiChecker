...
class BertLitModel(BaseLitModel):
    ...
    def __init__(self, model, args, tokenizer):
        ...
        self.eval_fn = f1_eval if "dialogue" in args.data_dir else partial(f1_score, rel_num=num_relation,
                                                                           na_num=Na_num)
        self.best_f1 = 0
    ...
    def _init_label_word(self, ):
        ...
        label_path = os.path.dirname(
            os.path.dirname(os.path.abspath(__file__))) + '/' + f"dataset/{model_name_or_path}_{dataset_name}.pt"
        label_path1 = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + '\\' + f'dataset\\roberta-base_ossl212.pt'
        # [num_labels, num_tokens], ignore the unanswerable
        if "dialogue" in args.data_dir:
            label_word_idx = torch.load(label_path1)[:-1]
        else:
            label_word_idx = torch.load(label_path1)
        with open(f"{args.data_dir}/rel2id.json", "r") as file:
            rel2id = json.load(file)
    ...
    def training_step(self, batch, batch_idx):  # pylint: disable=unused-argument
        input_ids = batch["input_ids"]  # 通过键访问 input_ids
        attention_mask = batch["attention_mask"]  # 通过键访问 attention_mask
        labels = batch["labels"]  # 通过键访问 labels
        so = batch["so"]  # 通过键访问 so
    ...
    def pvp(self, logits, input_ids):
        ...
        if mask_token_id not in input_ids:
            raise ValueError(f"mask_token_id {mask_token_id} not found in input_ids")

        _, mask_idx = (input_ids == mask_token_id).nonzero(as_tuple=True)
        if len(mask_idx) == 0:
            raise ValueError("No mask token found in input_ids")

        bs = input_ids.shape[0]

        if bs > len(mask_idx.detach().cpu().numpy()):
            bs = len(mask_idx.detach().cpu().numpy())
            print('!!!!! (pvp: mask_idx < input_ids :modify)')

        # print(logits.size())
        # print(input_ids.size(), bs)
        # print(mask_idx)
        if mask_idx.max() >= logits.shape[1]:
            raise ValueError(f"mask_idx {mask_idx.max()} is out of bounds for logits with shape {logits.shape}")

        mask_output = logits[torch.arange(
            bs), mask_idx]  ### shape mismatch: indexing tensors could not be broadcast together with shapes
    ...
    def ke_loss(self, logits, labels, so, input_ids):
        ...
        bsz = logits.shape[0]
        for i in range(bsz):
            if so[i][0] < 0 or so[i][1] > logits[i].shape[0]:
                raise ValueError(f"Invalid subject index for sample {i}: {so[i][0]}:{so[i][1]}")
            if so[i][2] < 0 or so[i][3] > logits[i].shape[0]:
                raise ValueError(f"Invalid object index for sample {i}: {so[i][2]}:{so[i][3]}")
            ...
            # 使用默认的CUDA随机数生成器
            max_len = logits[i].shape[0]
            span_sub = torch.randint(1, 6, (1,), device=logits.device,generator=generator).item()
            st_sub = torch.randint(0, logits[i].shape[0] - span_sub + 1, (1,), device=logits.device,generator=generator).item()
            span_obj = torch.randint(1, 6, (1,), device=logits.device,generator=generator).item()
            st_obj = torch.randint(0, logits[i].shape[0] - span_obj + 1, (1,), device=logits.device,generator=generator).item()
    ...
    def on_train_start(self):
        # 直接通过 Trainer 的内部属性获取总步数
        max_epochs = self.trainer.max_epochs
        total_batches = self.trainer.num_training_batches
        self.total_steps = max_epochs * total_batches
        ...
        self.trainer.lr_schedulers[0]["scheduler"] = scheduler
