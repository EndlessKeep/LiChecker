...
class LlamaLicensePredictor:
    def __init__(self, model_name="LLM-Research/Meta-Llama-3-8B-Instruct"):
        """
        初始化Llama模型预测器
        """
        ...
        '''except Exception as e:
            ...
            print("尝试使用备用模型...")
            # 备用模型
            backup_model = "modelscope/llama2-7b-chat"
            try:
                self.tokenizer = AutoTokenizer.from_pretrained(backup_model, cache_dir='./modelscope_cache')
                self.model = AutoModelForCausalLM.from_pretrained(backup_model, cache_dir='./modelscope_cache')
                if self.tokenizer.pad_token is None:
                    self.tokenizer.pad_token = self.tokenizer.eos_token
            except:
                raise Exception("无法加载任何Llama模型")'''
        ...
        # 标签映射
        self.label_mapping = {
            "允许": 1, "可以": 1, "permitted": 1, "allowed": 1, "yes": 1,
            "禁止": 2, "不允许": 2, "forbidden": 2, "prohibited": 2, "no": 2,
            "必须": 3, "要求": 3, "required": 3, "mandatory": 3, "must": 3,
            "不适用": 0, "无关": 0, "not applicable": 0, "n/a": 0, "na": 0
        }
    ...
    def create_prompt(self, license_text, term):
        ...
        创建用于Llama预测的提示词
        """
        prompt = f"""<s>[INST] You are a legal expert analyzing software licenses. Please analyze the following license text and determine the stance on the term "{term}".

License text:
{license_text}

Based on the license content, what is the stance on "{term}"? Please respond with only one number:
0 - Not applicable/Not mentioned
1 - Permitted/Allowed
2 - Forbidden/Prohibited
3 - Required/Mandatory

Response (number only): [/INST]"""
        return prompt
...
def main():
    ...
    output_file = "/root/autodl-tmp/predict_llama7b.csv"
    
    # 创建预测器
    predictor = LlamaLicensePredictor()
